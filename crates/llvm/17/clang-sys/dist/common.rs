...
impl Drop for CommandErrorPrinter {
    fn drop(&mut self) {
        ...
        let errors: HashMap<String, Vec<String>> = COMMAND_ERRORS.with(|e| e.borrow().clone());
...
pub fn run_llvm_config(arguments: &[&str]) -> Option<String> {
    ...
    let formatted: &str = &format!("{}", llvm_config_path.display());

    run_command("llvm-config", formatted, arguments)
}
...
fn search_directory(directory: &Path, filenames: &[String]) -> Vec<(PathBuf, String)> {
    ...
    let directory: String = Pattern::escape(directory.to_str().unwrap());
...
fn search_directories(directory: &Path, filenames: &[String]) -> Vec<(PathBuf, String)> {
    let mut results: Vec<(PathBuf, String)> = search_directory(directory, filenames);
    ...
    if target_os!("windows") && directory.ends_with("lib") {
        let sibling: PathBuf = directory.parent().unwrap().join("bin");
