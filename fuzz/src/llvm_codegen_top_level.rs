...
fn gen_expr<'ast>(
    ...
    match u.int_in_range(0..=upper)? {
        0 => self::gen_literal(u),
        1 if has_vars => self::gen_reference(u, scope),
        1 | 2 => Ok(Ast::BinaryOp {
            left: Box::new(self::gen_expr(u, scope, depth - 1)?),
            operator: u.arbitrary()?,
            right: Box::new(self::gen_expr(u, scope, depth - 1)?),
            kind: u.arbitrary()?,
            span: u.arbitrary()?,
            id: NodeId::new(),
        }),
        3 => Ok(Ast::UnaryOp {
            operator: u.arbitrary()?,
            kind: u.arbitrary()?,
            node: Box::new(self::gen_expr(u, scope, depth - 1)?),
            before: u.arbitrary()?,
            span: u.arbitrary()?,
            id: NodeId::new(),
        }),
        ...
        5 => {
            let from = Box::new(self::gen_expr(u, scope, depth - 1)?);
        ...
        6 if has_vars => Ok(Ast::Deref {
            value: Box::new(self::gen_reference(u, scope)?),
            kind: u.arbitrary()?,
        ...
        7 if has_vars => Ok(Ast::Load {
            source: Box::new(self::gen_reference(u, scope)?),
            kind: u.arbitrary()?,
            modificators: u.arbitrary()?,
        ...
        8 => self::gen_call_stmt(u, scope, depth),
        _ => self::gen_literal(u),
    }
}
...
fn gen_for<'ast>(
    ...
    let loop_var_name = self::gen_name(u)?;
    ...
    let condition = Box::new(Ast::BinaryOp {
        left: Box::new(Ast::Reference {
            ...
            kind: loop_var_kind.clone(),
            metadata: self::reference_metadata_for(&loop_var, false),
            span: u.arbitrary()?,
            id: NodeId::new(),
        }),
        operator: u.arbitrary()?,
    ...
    let actions = Box::new(self::gen_increment(u, &loop_var)?);
    let block = Box::new(self::gen_block(u, scope, depth.saturating_sub(1))?);
...
fn gen_function<'ast>(
    ...
    let body = Some(Box::new(self::gen_function_body(
        u,
        scope,
    ...
    Ok(Ast::Function {
        name: name.to_string(),
        ascii_name: name.to_string(),
        demangling_name: format!("fuzz.{name}"),
...
fn gen_function_body<'ast>(
    ...
    for _ in 0..n_stmts {
        nodes.push(self::gen_stmt(u, scope, depth)?);
    }
    ...
    if should_return {
        let expression = if is_void {
            ...
            Some(Box::new(self::gen_expr(u, scope, MAX_EXPR_DEPTH)?))
        };
    ...
    for _ in 0..n_post {
        post.push(self::gen_defer(u, scope, depth)?);
    }
...
fn gen_stmt<'ast>(
    ...
    match choice {
        0 => self::gen_var(u, scope, depth),
        1 => self::gen_const(u, scope),
        2 | 3 => self::gen_if(u, scope, depth),
        4 | 5 => self::gen_for(u, scope, depth),
        6 | 7 => self::gen_while(u, scope, depth),
        8 | 9 => self::gen_loop(u, scope, depth),
        10 | 11 => self::gen_block(u, scope, depth - 1),
        12 => self::gen_static(u, scope),
        13 => self::gen_mutation(u, scope, depth),
        14 => self::gen_call_stmt(u, scope, depth),
        15 => self::gen_return(u, scope, depth),
        16 => self::gen_defer(u, scope, depth),
        17 => self::gen_loop_control(u),
        _ => self::gen_reference(u, scope),
    }
}
...
fn gen_increment<'ast>(
    ...
    let current = Ast::Reference {
        name: name.to_string(),
        kind: kind.clone(),
        metadata: self::reference_metadata_for(var, true),
        span: u.arbitrary()?,
        id: NodeId::new(),
    };
...
fn gen_if<'ast>(
    ...
    let condition = Box::new(self::gen_condition(u, scope)?);
    let then_branch = Box::new(self::gen_block(u, scope, depth - 1)?);

    let n_elif = u.int_in_range(0..=2usize)?;
    let mut else_if_branch = Vec::with_capacity(n_elif);
    for _ in 0..n_elif {
        let elif_condition = Box::new(self::gen_condition(u, scope)?);
        let elif_block = Box::new(self::gen_block(u, scope, depth - 1)?);
    ...
    let else_branch = if u.arbitrary()? {
        Some(Box::new(Ast::Else {
            block: Box::new(self::gen_block(u, scope, depth - 1)?),
            kind: u.arbitrary()?,
            span: u.arbitrary()?,
            id: NodeId::new(),
        }))
...
fn gen_condition<'ast>(
    ...
    if scope.has_any() {
        ...
        let left = Ast::Reference {
            ...
            metadata: self::reference_metadata_for(&picked, false),
            span: u.arbitrary()?,
            id: NodeId::new(),
        };
...
fn gen_var<'ast>(
    ...
    let value = if u.arbitrary()? {
        Some(Box::new(self::gen_expr(u, scope, depth.saturating_sub(1))?))
    } else {
        None
    };
...
fn gen_while<'ast>(
    ...
    let variable = if u.arbitrary()? {
        Some(Box::new(self::gen_var(u, scope, depth.saturating_sub(1))?))
    } else {
        None
    };
...
fn gen_mutation<'ast>(
    ...
    let Some(picked) = scope.pick_mutable(u)? else {
        return self::gen_var(u, scope, depth);
    };
    ...
    Ok(Ast::Mutation {
        ...
        value: Box::new(self::gen_expr(u, scope, depth.saturating_sub(1))?),
        kind,
        span: u.arbitrary()?,
        id: NodeId::new(),
    })
...
fn gen_call_stmt<'ast>(
    ...
    Ok(Ast::Call {
        name: self::gen_name(u)?.to_string(),
        args,
...
fn gen_const<'ast>(
    ...
    let name = self::gen_name(u)?;
    let kind: Type = u.arbitrary()?;
    let value = Box::new(self::gen_expr(u, scope, MAX_EXPR_DEPTH)?);
...
fn reference_metadata_for(var: &ScopedVar<'_>, is_mutable: bool) -> ReferenceMetadata {
    match var.reference_type {
        ...
        ReferenceType::Static => {
            ReferenceMetadata::new(true, is_mutable, ReferenceType::Static, false, None)
        }
        ReferenceType::Constant => {
            ReferenceMetadata::new(true, false, ReferenceType::Constant, false, None)
        }
        _ => ReferenceMetadata::new(true, is_mutable, ReferenceType::Local, false, None),
    }
}
...
fn gen_loop<'ast>(
    ...
    Ok(Ast::Loop {
        block: Box::new(self::gen_block(u, scope, depth.saturating_sub(1))?),
        kind: u.arbitrary()?,
        span: u.arbitrary()?,
        id: NodeId::new(),
    })
...
fn gen_defer<'ast>(
    ...
    Ok(Ast::Defer {
        node: Box::new(self::gen_stmt(u, scope, depth.saturating_sub(1))?),
        kind: u.arbitrary()?,
        span: u.arbitrary()?,
        id: NodeId::new(),
    })
...
pub fn gen_root<'ast>(u: &mut Unstructured<'ast>) -> arbitrary::Result<Ast<'ast>> {
    ...
    self::gen_function(u, &mut scope, MAX_DEPTH)
}
